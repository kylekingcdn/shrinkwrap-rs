...
#[shrinkwrap(schema, inline, transform = MyTransform, fallible(error = MyError))]
...
impl TryTransformToNest<ApiDataNestedText> for MyTransform {
    type Data = ApiData;
    type Error = MyError;
    ...
    ) -> Result<ApiDataNestedText, MyError> {
        Ok(ApiDataNestedText {
...
impl TryTransformToNest<ApiDataNestedValue> for MyTransform {
    type Data = ApiData;
    type Error = MyError;
    ...
    fn try_transform_to_nest(
        ...
        _data: &ApiData,
        _options: &MyTransformOpts,
    ) -> Result<ApiDataNestedValue, MyError> {
        Ok(ApiDataNestedValue { balance: 6.66f32 })
    }
}

/// Define the conversion from our **`value`** nest into the generated `value_text` sub-nest
impl TryTransformToNest<ApiDataNestedValueText> for MyTransform {
    ...
    fn try_transform_to_nest(
        ...
        _options: &MyTransformOpts,
    ) -> Result<ApiDataNestedValueText, MyError> {
        Ok(ApiDataNestedValueText {
