...
#[shrinkwrap(schema, inline, transform = MyTransform, all_optional)]
...
impl TransformToNest<Option<ApiDataNestedText>> for MyTransform {
    type Data = ApiData;
    ...
    ) -> Option<ApiDataNestedText> {
        options.with_text.then_some(ApiDataNestedText {
...
impl TransformToNest<Option<ApiDataNestedValue>> for MyTransform {
    type Data = ApiData;
...
impl TransformToNest<Option<ApiDataNestedValueText>> for MyTransform {
    ...
    fn transform_to_nest(
        ...
        options: &MyTransformOpts,
    ) -> Option<ApiDataNestedValueText> {
        options.with_text.then_some(ApiDataNestedValueText {
...
pub fn main() -> Result<(), serde_json::Error> {
    ...
    let wrapped = data.to_wrapped_with(&global_transform, &transform_opts);
