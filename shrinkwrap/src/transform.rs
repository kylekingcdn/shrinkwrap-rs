...
/// The only real change is instead of using the primary data source (`MyData`) for the first generic parameter, you would use the parent nest.
///
...
/// ## Notes
///
/// When a nest (e.g. `NestA`) has children nests layered under it (deeply nested), it's type will be swapped out with a wrapper (`NestAWrapper`) within `NestA`'s parent.
///
...
///
/// This is the reason that the [`TransformToNest`] impls for deeply nested structs requires so few adjustments and why there is no impl over `NestAWrapper`
/// - it's already done for you). You just need to implement the mapping of `NestA` -> `TheChildNestB` - which is exactly what is shown above.
///
...
///
/// Users should instead implement [`TransformToNest`]. This trait is interoperable and can be implemented on any struct, not just Data structs.
/// This allows for dependency injection and/or state management.
///
...
/// `ToWrappedWith` is automatically implemented when all top-level nests have a [`TransformToNest`] impl.
///
...
///
/// This allows for converting data from the desired wrapper type.
